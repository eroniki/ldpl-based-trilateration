...
map.map = map.map';
%% Parse the data
db = importfile('measurements/1477330902.txt', 1, 1985);

% append timestamps
db_(:,1) = table2cell(db(:,1));
% append mac addresses 
db_(:,2) = table2array(db(:,2));
% append rssi 
db_(:,3) = table2cell(db(:,3));
% append ssid
db_(:,4) = table2cell(db(:,4));
% append x position
db_(:,5) = table2cell(db(:,5));
% append y position
db_(:,6) = table2cell(db(:,6));
% create cov matrices
...
for i=1:nRows
    % get rows
    buf = table2cell(db(i,7));
    % convert to mat
    buf = cell2mat(buf);
    % delete leading and following parantheses
    buf(1) = [];
    buf(end) = [];
    % create a 6x6 mat
    cov = reshape(strread(buf, '%f', 'delimiter', ','), [6,6]);
    db_(i,7) = mat2cell(cov, 6,6);
end
...
for jj=1:nTimeStamp
    rows = db.timestamp == uniqueTimeStamps(jj);
    cols = {'x','y'};
    tmp = table2array(db(rows,cols));
    pos(jj,:) = tmp(1,:);
end

for row = 1:nRows
    mac = db(row,'mac');
    rssi = table2array(db(row, 'rssi'));

    ts = int64(table2array(db(row, 'timestamp')));
...
training_id = randsample(1:rows, 72);
...
px = round((pos_testing(:,1)-map.origin.x)/map.res);
py = round((pos_testing(:,2)-map.origin.y)/map.res);

p_hat_x = round((pos_predict_x-map.origin.x)/map.res);
p_hat_y = round((pos_predict_y-map.origin.y)/map.res);
colors = rand(length(pos_testing),3);
% showPointCloud([px, py, repmat(0, length(p_hat_x))]);

for jj=1:length(pos_testing)
    figure; imshow(map.map); title('Testing Location vs Predicted Locations'); hold on;
    plot(py(jj), px(jj), 'g*'); hold on;
    plot(p_hat_y(jj), p_hat_x(jj), 'r*');
...
for jj=1:n
    saveas(jj, ['outputs/goodwin/', num2str(jj), '.jpg']);
end
